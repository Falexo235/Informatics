...
int main() {
    ...
    vector<double> x = linspace(-3, 3);
    vector<double> y1 = transform(x, [lines](auto x) { return (x - lines[0][2]) / lines[0][1]; });
    vector<double> y2 = transform(x, [lines](auto x) { return (x - lines[1][2]) / lines[1][1]; });
    plot(x, y1, "g", x, y2, "-o");
    show();
