...
begin
 if (k<1) or (k>5) then exit;
  i:=1;
  if k<4 then begin
     while i<(length(ListForm.A1i)-1) do begin
      if ListForm.A2[ListForm.A2i[i],k]<=ListForm.A2[ListForm.A2i[i+1],k] then i:=i+1
      else begin
        c:=ListForm.A1i[i]; ListForm.A1i[i]:= ListForm.A1i[i+1]; ListForm.A1i[i+1]:=c;
        c:=ListForm.A2i[i]; ListForm.A2i[i]:= ListForm.A2i[i+1]; ListForm.A2i[i+1]:=c;
        i:=i-1;
    end;
      end;
     end
      ...
      while i<(length(ListForm.A1i)-1) do
       if ListForm.A1[ListForm.A1i[i],k]<=ListForm.A1[ListForm.A1i[i+1],k] then i:=i+1
      else begin
        c:=ListForm.A1i[i]; ListForm.A1i[i]:= ListForm.A1i[i+1]; ListForm.A1i[i+1]:=c;
        c:=ListForm.A2i[i]; ListForm.A2i[i]:= ListForm.A2i[i+1]; ListForm.A2i[i+1]:=c;
        i:=i-1;
...
begin
  ...
  for i:=1 to length(A2i)-1 do begin
  ...
  FileMemo.Lines.Add('Таблица сортирована по столбцу '+inttostr(k));
end;
...
begin
  ...
  i:=1;
  for k:=1 to length(A2i)-1 do begin
   if (A1[A1i[k],2] <> 'Россия') then begin
...
begin
  ...
  sortat(5);
  sortat(4);
for i:=1 to length(A1i)-1 do begin
    if (A1[A1i[i],2] = 'Россия') and (A1[A1i[i],3] <> A1[A1i[i+1],3]) then ListBox1.Items.Add(A1[A1i[i],3]);
end;
FileMemo.Lines.Clear;
...
begin
  ...
  if FileOpenDialog.Execute then begin
    if FileExists(FileOpenDialog.FileName) then begin
     ...
     i:=1;
     while i<(length(A1i)-1) do begin
      if A1[A1i[i],1]<=A1[A1i[i+1],1] then i:=i+1
      else begin
        c:=A1i[i]; A1i[i]:= A1i[i+1]; A1i[i+1]:=c;
        i:=i-1;
    end;
      end;
      i:=1;
     while i<(length(A2i)-1) do begin
      if A2[A2i[i],1]<=A2[A2i[i+1],1] then i:=i+1
      else begin
        c:=A2i[i]; A2i[i]:= A2i[i+1]; A2i[i+1]:=c;
        i:=i-1;
    end;
      end;
end;
...
while A1[A1i[i],2]='Россия' do begin
 if (A1[A1i[i],3])<>(A1[A1i[i+1],3]) then begin
 ...
 else while A1[A1i[i],3]=A1[A1i[i+1],3] do i:=i+1;
 i:=i+1;
end;
FileMemo.Lines.Clear;
FileMemo.Lines.Add('Выведен список субъектов с 1 студентом и год его рождения');
