...
uses
  ...
  StdCtrls, FileUtil, LResources, ShellAPI, Windows, ExtCtrls;
...
begin
  ...
  ListForm.FileMemo.Lines.Add(
    'Укажите файл для сохранения списка');
  if ListForm.FileSaveDialog.Execute then
  begin
    ...
    ListForm.FileMemo.Lines.Clear;
    ListForm.FileMemo.Lines.Add('Список записан в файл ' +
      ListForm.FileSaveDialog.FileName);
  end
  else;
end;
...
begin
  ...
  begin
    for i := 1 to 40 do ListBox2.Items.Add(IntToStr(B[i]));
    ListBox2.Items.Add('...');
  end;
  setlength(Y, 0);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ListForm.FileMemo.Lines.Clear;
        ListForm.FileMemo.Lines.Add('Ошибка чтения файла ' +
